...
/*   Updated: 2018/02/26 15:56:29 by kyork            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_fork(t_vm *vm, t_proc *proc) {
	...
	offset = ((t_s32)proc->decoded_args[0]) % IDX_MOD;
	dest = mem_index(proc->pc, (ssize_t)offset);
...
void	op_lfork(t_vm *vm, t_proc *proc) {
	...
	offset = (t_s32)proc->decoded_args[0];
	dest = mem_index(proc->pc, (ssize_t)offset);
